...
    }
    ...
    async function exportConversations(options = {}) {
        ...
        try {
            ...
            showExportResult(stats, failures, options);
            btn.textContent = '✅ 完成';
     ...
     * [重构] 多步骤、用户主导的导出对话框
     */
    function dashboardShell(title) {
        document.getElementById('gpt-dashboard-overlay')?.remove();
        const overlay = document.createElement('div'); overlay.id = 'gpt-dashboard-overlay';
        Object.assign(overlay.style, {position:'fixed', inset:'0', zIndex:100000, background:'rgba(15,35,28,.48)', display:'grid', placeItems:'center', padding:'20px'});
        const host = document.createElement('div'); overlay.append(host);
        const root = host.attachShadow({mode:'open'});
        root.innerHTML = `<style>
        *{box-sizing:border-box}section{background:#f6f8f4;color:#18312d;border:1px solid #dce5df;border-radius:20px;padding:28px;width:min(680px,calc(100vw - 40px));max-height:85vh;overflow:auto;font:14px/1.6 -apple-system,BlinkMacSystemFont,sans-serif;box-shadow:0 24px 90px #0003}h2{font-size:27px;margin:6px 0 18px}p{color:#667b71}small{font-size:11px;letter-spacing:2px;color:#176b50}button{font:inherit;background:white;color:#18312d;border:1px solid #dce5df;padding:10px 16px;border-radius:9px;cursor:pointer}button:focus-visible,input:focus-visible{outline:3px solid #79ba9f}button.primary{background:#176b50;color:white}button:disabled{opacity:.5;cursor:wait}header,.actions{display:flex;gap:10px;align-items:center;flex-wrap:wrap}header{justify-content:space-between}.card{padding:16px;background:white;border:1px solid #dce5df;border-radius:12px;margin:14px 0}input{width:100%;padding:11px;border:1px solid #dce5df;border-radius:8px;color:#18312d;background:white;font:inherit;margin:8px 0}.list{max-height:260px;overflow:auto}.list button{display:block;width:100%;text-align:left;margin:6px 0}.error{color:#923e28;white-space:pre-wrap}
        </style><section role="dialog" aria-modal="true" aria-label="GPT Exporter"><header><small>GPT EXPORTER / 2.0</small><button id="close" aria-label="关闭">关闭</button></header><h2></h2><div id="body"></div></section>`;
        ...
        overlay.onclick=e=>{if(e.target===overlay)close();};
        root.addEventListener('keydown',e=>{if(e.key==='Escape')close();if(e.key==='Tab'){const els=[...root.querySelectorAll('button:not([disabled]),input')];const first=els[0],last=els.at(-1);if(e.shiftKey&&root.activeElement===first){e.preventDefault();last.focus();}else if(!e.shiftKey&&root.activeElement===last){e.preventDefault();first.focus();}}});
        document.body.append(overlay);root.querySelector('#close').focus();
        return {root,body:root.querySelector('#body'),close};
    }
    function showExportDialog() {
        const {root,body,close}=dashboardShell('备份你的对话');
        body.innerHTML=`<p>JSON 保留完整对话；Markdown 只导出当前可见分支。</p><div class="card"><strong>Conversations</strong><label><input id="workspace" placeholder="Workspace ID（留空使用当前工作区）" aria-label="Workspace ID"></label><div class="actions"><button class="primary" id="pick">选择对话</button><button id="all">完整备份</button><button id="projects">查看 Projects</button></div></div><div class="card"><strong>缓存与 Sync</strong><p id="cache">正在读取缓存…</p><p id="sync"></p><button id="sync-center">打开 Notion 同步中心</button><p>增量扫描、失败重试、连接配置和完整同步记录在扩展同步中心管理。</p></div><div class="list" id="project-list"></div><p class="error" id="error" role="status"></p><button id="advanced">更多导出工具</button>`;
        const workspace=()=>root.querySelector('#workspace').value.trim()||null;
        root.querySelector('#workspace').value=resolveWorkspaceId(null)||'';
        ExportCache.size().then(n=>root.querySelector('#cache').textContent=`缓存 ${n} 条 · 使用服务器更新时间校验；没有可靠时间时重新拉取`);
        root.querySelector('#sync').textContent=document.documentElement.getAttribute('data-gpt-sync-summary')||'尚无同步状态，请打开同步中心';
        root.querySelector('#sync-center').onclick=()=>{document.dispatchEvent(new CustomEvent('GPT_OPEN_SYNC_UI'));};
        root.querySelector('#pick').onclick=()=>{const id=workspace();close();showConversationPicker({mode:id?'team':'personal',workspaceId:id});};
        root.querySelector('#all').onclick=()=>{const id=workspace();close();startExportProcess(id?'team':'personal',id,true);};
        root.querySelector('#advanced').onclick=()=>{close();showLegacyExportDialog();};
        root.querySelector('#projects').onclick=async()=>{
            const button=root.querySelector('#projects');button.disabled=true;
            try {
                if(!await ensureAccessToken())throw new Error('请先登录 ChatGPT');
                const projects=await getProjectSpaces(workspace(),{ownedOnly:false});
                const list=root.querySelector('#project-list');list.replaceChildren();
                for(const project of projects){const b=document.createElement('button');b.textContent=`${project.title} · ${project.id}`;b.onclick=()=>{const id=workspace();close();startSingleProjectExportProcess(project,id,true);};list.append(b);}
                if(!projects.length)list.textContent='当前列表未返回项目。';
            }catch(e){root.querySelector('#error').textContent=e.message;}finally{button.disabled=false;}
        };
    }
    function showExportResult(stats, failures, options) {
        const {body,close}=dashboardShell(stats.failed?'导出完成，部分失败':'备份已生成');
        const summary=document.createElement('p');summary.textContent=`缓存命中 ${stats.hits} · 新拉取 ${stats.fetched} · 失败 ${stats.failed}。请在浏览器中完成文件保存。`;body.append(summary);
        for(const failure of failures){const p=document.createElement('p');p.className='error';p.textContent=`${failure.title||failure.id}：${failure.error}`;body.append(p);}
        if(failures.length){const retry=document.createElement('button');retry.className='primary';retry.textContent='仅重试失败项（生成补充 ZIP）';retry.onclick=()=>{close();const entries=failures.map(f=>options.conversationEntries?.find(e=>e.id===f.id)||{id:f.id,title:f.title});exportConversations({...options,conversationEntries:entries,useCache:false});};body.append(retry);}
    }

    function showLegacyExportDialog() {
    ...
    async function scanPageForSync(workspaceId, checkpoint = null) {
        if (!workspaceId || !await ensureAccessToken()) throw new Error('缺少工作区或登录状态');
        const state = checkpoint || {stage:'root', archived:false, offset:0, cursor:null, seen:[], projects:[], projectIndex:0};
        const headers = {Authorization:`Bearer ${accessToken}`, 'oai-device-id':getOaiDeviceId(), 'ChatGPT-Account-Id':workspaceId};
        if (!headers['oai-device-id']) throw new Error('缺少设备 ID，请刷新页面');
        const checkedFetch = async url => {
            const response = await fetch(url,{headers,signal:AbortSignal.timeout(25000)});
            if (!response.ok) {
                const error = new Error(`扫描 ${state.stage} 失败 (${response.status})`);
                error.retryable = response.status === 429 || response.status >= 500;
                error.retryAfter = Number(response.headers.get('retry-after')) || 10;
                throw error;
            }
            return response.json();
        };
        let items = [], projects = [], next = {...state};
        if (state.stage === 'root') {
            const query = new URLSearchParams({offset:String(state.offset),limit:String(PAGE_LIMIT),order:'updated'});
            if(state.archived)query.set('is_archived','true');
            if(state.cursor)query.set('cursor',state.cursor);
            const data=await checkedFetch(`/backend-api/conversations?${query}`);
            items=responseItems(data);const offset=state.offset+items.length;
            const cursor=responseCursor(data), more=!!cursor||offsetHasMore(data,items,offset);
            const fingerprint=JSON.stringify(items.map(it=>normalizeConversationListItem(it)?.id));
            if(items.length&&state.seen.includes(fingerprint))throw new Error('对话列表重复分页，未推进扫描检查点');
            if(!items.length&&more)throw new Error('对话列表提前返回空页');
            const map=new Map();items.forEach(it=>upsertConversationEntry(map,it,{is_archived:state.archived}));items=[...map.values()];
            next={...state,offset,cursor,seen:[...state.seen,fingerprint]};
            if(!more)next=state.archived?{...state,stage:'sidebar',cursor:null,seen:[]}:{...state,archived:true,offset:0,cursor:null,seen:[]};
        } else if(state.stage === 'sidebar') {
            const query=new URLSearchParams({limit:String(PROJECT_SIDEBAR_LIMIT),conversations_per_gizmo:'0',owned_only:'false'});
            if(state.cursor)query.set('cursor',state.cursor);
            const data=await checkedFetch(`/backend-api/gizmos/snorlax/sidebar?${query}`);
            projects=responseItems(data).map(normalizeProjectSpaceItem).filter(Boolean).map(p=>({id:p.id,title:p.title}));
            const cursor=responseCursor(data);
            if(cursor&&state.seen.includes(cursor))throw new Error('项目列表重复游标');
            if(!cursor&&responseHasMore(data,false))throw new Error('项目列表缺失下一页游标');
            const merged=[...new Map([...state.projects,...projects].map(p=>[p.id,p])).values()];
            next={...state,projects:merged,cursor,seen:[...state.seen,cursor]};
            if(!cursor)next=merged.length?{...next,stage:'project',cursor:null,seen:[]}:null;
        } else if(state.stage === 'project') {
            const project=state.projects[state.projectIndex];
            const result=await fetchProjectGizmoConversationPage(project,headers,state.cursor);
            if(!result.ok)throw new Error(`项目 ${project.title} 分页不可用；根列表可证明的归属已保留，但不能将本次扫描标为完整`);
            const data=result.data, raw=responseItems(data), cursor=responseCursor(data);
            const fingerprint=JSON.stringify(raw.map(it=>normalizeConversationListItem(it)?.id));
            if(raw.length&&state.seen.includes(fingerprint))throw new Error('项目对话重复分页');
            if(!cursor&&responseHasMore(data,false))throw new Error('项目对话缺少下一页游标');
            if(cursor&&!raw.length)throw new Error('项目对话提前返回空页');
            const map=new Map();raw.forEach(it=>upsertConversationEntry(map,it,{projectId:project.id,projectTitle:project.title}));items=[...map.values()];
            next={...state,cursor,seen:[...state.seen,fingerprint]};
            if(!cursor)next=state.projectIndex+1<state.projects.length?{...state,projectIndex:state.projectIndex+1,cursor:null,seen:[]}:null;
        } else throw new Error('扫描检查点无效');
        return {entries:items,projects,next,cacheSize:await ExportCache.size()};
    }
